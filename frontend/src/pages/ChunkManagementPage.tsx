...
import { Settings, RefreshCcw, Plus, X, Check, AlertTriangle, Volume2, Play, Pause, FileText, ExternalLink, Music, Upload } from 'lucide-react';
...
const api = {
  ...
  updateChunkFromFile: async (chunkId: number) => {
    const response = await fetch(`/api/chunks/${chunkId}/update-from-file`, { method: 'POST' });
    if (!response.ok) throw new Error('Failed to update chunk from file');
    return response.json();
  }
};
...
export const ChunkManagementPage: React.FC = () => {
  ...
  const [chunkTimeRanges, setChunkTimeRanges] = useState<Map<number, { start: number; end: number }>>(new Map());
  ...
  const loadChapterStatus = async () => {
    try {
      ...
      // Also fetch accurate time ranges from audio-sync-data API
      await loadChunkTimeRanges();
    } catch (err: any) {
  ...
  const loadChunkTimeRanges = async () => {
    try {
      const response = await fetch(`/api/chapters/${chapterId}/audio-sync-data`);
      if (!response.ok) throw new Error('Failed to fetch audio sync data');
      const syncData = await response.json();
      
      // Create a map of chunk_id to time ranges
      const timeRangeMap = new Map<number, { start: number; end: number }>();
      if (syncData.chunk_boundaries) {
        syncData.chunk_boundaries.forEach((chunk: any) => {
          timeRangeMap.set(chunk.chunk_id, {
            start: chunk.start_time,
            end: chunk.end_time
          });
        });
      }
      setChunkTimeRanges(timeRangeMap);
    } catch (err: any) {
      console.warn('Failed to load chunk time ranges:', err.message);
  ...
  const playAudio = async (chunkId: number) => {
    try {
      ...
      // Create or get audio element - use the API endpoint which serves the correct file
      ...
      if (!audio) {
        ...
        audio.addEventListener('error', () => {
          setError('Failed to load audio - check that the audio file path is correct');
          setPlayingChunk(null);
        });
      ...
      setError('Failed to play audio - verify the audio file exists at the displayed path');
      setPlayingChunk(null);
    }
  };
  ...
  const handleUpdateFromFile = async (chunkId: number) => {
    try {
      setProcessing(prev => new Set(prev).add(chunkId));
      const result = await api.updateChunkFromFile(chunkId);
      
      // Show success message briefly
      const successMessage = `Updated chunk ${result.chunk_number || chunkId} from file (${result.text_length} characters)`;
      setError(null);
      
      // Reload chapter status to show updated data
      setTimeout(loadChapterStatus, 1000);
      
      // Clear success message after a few seconds
      setTimeout(() => {
        if (error === null) {
          // Only clear if no new error has occurred
        }
      }, 3000);
        ...
        <div className="layout-content-container flex flex-col w-[960px] max-w-[960px] py-5">
          ...
          <div className="bg-white border border-primary-200 rounded-lg overflow-hidden">
            ...
            <div className="divide-y divide-primary-100">
              {chapterStatus.chunks.map((chunk, arrayIndex) => (
                <div 
                  key={chunk.chunk_id} 
                  className={`p-4 hover:bg-primary-25 ${
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => handleUpdateFromFile(chunk.chunk_id)}
                        disabled={processing.has(chunk.chunk_id)}
                        className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        title="Update chunk text from file system"
                      >
                        <Upload className={`h-4 w-4 ${processing.has(chunk.chunk_id) ? 'animate-spin' : ''}`} />
                      </button>
                  ...
                  {/* Audio file path and time info - displayed below buttons */}
                  <div className="mt-3 space-y-1">
                    {/* Audio file path */}
                    {chunk.audio_file_path && (
                      <div className="text-xs text-gray-500 font-mono bg-gray-50 p-2 rounded border">
                        <span className="font-medium text-gray-700">Audio File:</span> {chunk.audio_file_path}
                      </div>
                    )}
                    
                    {/* Time positions - using accurate data from audio-sync-data API */}
                    {chunk.has_audio && (
                      <div className="text-xs text-gray-600 bg-blue-50 p-2 rounded border">
                        <span className="font-medium text-gray-700">Time Range:</span> {(() => {
                          const timeRange = chunkTimeRanges.get(chunk.chunk_id);
                          if (timeRange) {
                            return `${formatTime(timeRange.start)} - ${formatTime(timeRange.end)}`;
                          } else {
                            // Fallback to word count estimation if sync data not available
                            let cumulativeTime = 0;
                            const currentChunkIndex = chapterStatus.chunks.findIndex(c => c.chunk_id === chunk.chunk_id);
                            
                            for (let i = 0; i < currentChunkIndex; i++) {
                              const prevChunk = chapterStatus.chunks[i];
                              if (prevChunk.has_audio) {
                                const estimatedDuration = (prevChunk.word_count / 150) * 60;
                                cumulativeTime += estimatedDuration;
                              }
                            }
                            
                            const currentChunkDuration = (chunk.word_count / 150) * 60;
                            const startTime = cumulativeTime;
                            const endTime = cumulativeTime + currentChunkDuration;
                            
                            return `${formatTime(startTime)} - ${formatTime(endTime)} (estimated)`;
                          }
                        })()}
                      </div>
                    )}
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <ul className="text-sm text-blue-800 space-y-1">
              ...
              <li>• 📤 Upload button: Update chunk database with latest text from file system</li>
